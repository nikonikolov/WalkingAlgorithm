...
*/
State_t::State_t(double height_in, BodyParams& robot_params) : servo_angles { 0.0 }, vars{ 0.0 }, params(robot_params) {
...
void State_t::updateVar(int* address, double value, bool update_state /*=true*/){
	
	*address = value;

	switch(int(address)){
		case int(&(vars.hip_to_end)):
			vars.hip_to_end_sq = pow(value, 2);
			break
    	case int(&(vars.height)):
    		vars.height_sq = pow(value, 2);
    		break
    	case int(&(vars.arm_ground_to_ef)):
    		vars.arm_ground_to_ef_sq = pow(value, 2);
    		break
    	case int(&(vars.ef_center)):
    		vars.ef_center_sq = pow(value, 2);
    		break
    	case int(&(vars.hip_to_end_sq)):
    		vars.hip_to_end = sqrt(value, 2);
    		break
    	case int(&(vars.height_sq)):
    		vars.height = sqrt(value, 2);
    		break
    	case int(&(vars.arm_ground_to_ef_sq)):
    		vars.arm_ground_to_ef = sqrt(value, 2);
    		break
    	case int(&(vars.ef_center_sq)):
    		vars.ef_center = sqrt(value, 2);
    		break
		default:
			printf("Error in State_t::updateVar\n\r");
	}

	if(update_state) update(base_idx);	
}

void State_t::updateVar(int* address, double value, int* address_sq, double value_sq){
	*address = value;
	*address_sq = value_sq;
	update(address);	
}
...
void State_t::update(int* address){
	switch(int(address)){
		case int(&(vars.hip_to_end)):
...
/*
	switch(offser){
		case offsetof(DynamicVars, hip_to_end):
			// Assumed that change is in END EFFECTOR and only arm_ground_to_ef is affected
			updateVar(vars.arm_ground_to_ef, sqrt(vars.hip_to_end_sq - vars.height_sq) + params.COXA, false);
			break;
		case offsetof(DynamicVars, arm_ground_to_ef):
			// Only hip_to_end is affected
			updateVar(vars.hip_to_end_sq, pow((vars.arm_ground_to_ef - params.COXA),2) + vars.height_sq, false);
			break;
		case offsetof(DynamicVars, height):
			// Only ef_center and hip_to_end affected. arm_ground_to_ef is not affected by height change
			vars.hip_to_end_sq = pow((vars.arm_ground_to_ef - params.COXA),2) + vars.height_sq;
			vars.hip_to_end = sqrt(vars.hip_to_end_sq) 
